...
export default function ApplicationCard({ app, onEdit, onDelete }) {
  return (
    <div className="group rounded-2xl border border-border/60 dark:border-border/70 bg-card/70 dark:bg-slate-800 p-5 backdrop-blur-xl shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-premium relative overflow-hidden">
      ...
      <div className="relative flex items-center justify-end gap-2 border-t border-border/50 dark:border-border/40 pt-4">
        <Link
          to={`/applications/${app.id}`}
          className="rounded-lg p-2 text-text-muted hover:bg-blue-100 dark:hover:bg-blue-500/30 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200 hover:scale-105 active:scale-95"
        >
          <Eye size={18} />
        </Link>
        <button
          onClick={() => onEdit(app)}
          className="rounded-lg p-2 text-text-muted hover:bg-amber-100 dark:hover:bg-amber-500/30 hover:text-amber-600 dark:hover:text-amber-400 transition-all duration-200 hover:scale-105 active:scale-95"
        >
          <Pencil size={18} />
        </button>
        <button
          onClick={() => onDelete(app)}
          className="rounded-lg p-2 text-text-muted hover:bg-red-100 dark:hover:bg-red-500/30 hover:text-red-600 dark:hover:text-red-400 transition-all duration-200 hover:scale-105 active:scale-95"
        >
          <Trash2 size={18} />
        </button>
      </div>
